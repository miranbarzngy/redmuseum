...
function CategoryCard({ category }: { category: GalleryCategoryRow }) {
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex items-start justify-center gap-3">
        <EditLink href={`/admin/gallery-categories/${category.id}`} showLabel />
        <DeleteButton
...
function AddCategoryTile() {
  return (
    ...
    >
      ...
      <span className="font-kurdish text-fluid-xs leading-tight text-ink-faint">زیادکردنی پۆل</span>
    </Link>
  );
}

/** Grid of gallery-category cards — slug, title, edit/delete actions — with
 * drag-to-reorder (persisted via reorderCategories) and a trailing "+" tile
 * to add a new category. Mirrors museumhistory/EventGrid, since categories
 * carry no cover photo either. */
export function CategoryGrid({ categories }: { categories: GalleryCategoryRow[] }) {
