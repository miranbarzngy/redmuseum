...
export function VisitorTypeForm({
  action,
  category,
}: {
  action: (formData: FormData) => Promise<void>;
  category?: BookingVisitorTypeRow;
}) {
  return (
    <LanguageProvider>
      <form action={action} className="flex flex-col gap-5">
        ...
        <SaveBar label={category ? "پاشەکەوتکردنی گۆڕانکارییەکان" : "زیادکردنی جۆر"} />
      </form>
    </LanguageProvider>
  );
}
